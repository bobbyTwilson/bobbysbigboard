...
function bbbUpdateCard(u,featured=false){
  const tc=bbbUpdateTypeClass(u.update_type),status=(u.injury_status||'').trim();
  return `<article class="bbb-update-card ${featured?'featured':''}" data-player-key="${bbbEsc(u.player_key)}">
    ...
    <div class="bbb-update-player"><div><h3>${bbbEsc(u.name)}</h3><div class="bbb-update-meta">${bbbEsc(u.pos||'')} ${u.rank?`• BBB #${bbbEsc(u.rank)}`:''} ${u.team?`• ${bbbEsc(u.team)}`:''}</div></div><span class="bbb-update-arrow">↗</span></div>
...
function bbbInjectUpdatesStyles(){
  ...
  .bbb-updates-home-copy,.bbb-updates-page-copy{max-width:540px;color:#87978e;font-size:13px}.bbb-updates-home-grid{display:grid;grid-template-columns:1.3fr .7fr;gap:15px}.bbb-update-stack{display:grid;gap:12px}.bbb-update-card{border:1px solid #193127;background:linear-gradient(160deg,#0c1511,#070b09);border-radius:15px;padding:18px;cursor:pointer;transition:.16s ease}.bbb-update-card:hover{transform:translateY(-1px);border-color:#28523f}.bbb-update-card.featured{padding:24px;min-height:100%}.bbb-update-top,.bbb-update-player{display:flex;justify-content:space-between;align-items:flex-start;gap:14px}.bbb-update-tags{display:flex;gap:7px;flex-wrap:wrap}.bbb-update-type,.bbb-update-status{display:inline-flex;border-radius:999px;padding:4px 8px;font-size:8px;font-weight:950;letter-spacing:.06em;text-transform:uppercase;border:1px solid #2b4b3d}.bbb-update-type.injury{background:#2c2210;border-color:#66511c;color:#f0d16f}.bbb-update-type.performance{background:#0a2b1d;border-color:#176743;color:#74e5a9}.bbb-update-type.role{background:#102232;border-color:#28506d;color:#8ec9f0}.bbb-update-type.roster{background:#21192f;border-color:#523d72;color:#c4a7eb}.bbb-update-type.other{background:#1a211e;color:#b6c0ba}.bbb-update-status{background:#231f10;border-color:#6c5c29;color:#e8cd74}.bbb-update-date{font-size:9px;color:#708178}.bbb-update-player{margin-top:16px;align-items:center}.bbb-update-player h3{margin:0;color:#fff;font-size:18px}.bbb-update-card.featured .bbb-update-player h3{font-size:25px}.bbb-update-meta{margin-top:4px;color:#71847a;font-size:9px;font-weight:850}.bbb-update-arrow{color:#50ce8e;font-size:18px}.bbb-update-card p{margin:14px 0 0;color:#bac7c0;font-size:12px;line-height:1.7}.bbb-update-card.featured p{font-size:14px}.bbb-updates-home-actions{margin-top:20px;display:flex;justify-content:flex-end}.bbb-updates-view{background:#050807;min-height:70vh}.bbb-updates-hero{padding:56px 0 34px;border-bottom:1px solid #173328;background:radial-gradient(circle at 78% 12%,rgba(10,143,77,.19),transparent 38%),linear-gradient(180deg,#07100c,#050807)}.bbb-updates-content{padding:54px 0 70px}.bbb-update-controls{display:grid;grid-template-columns:auto minmax(220px,1fr) 150px;gap:12px;margin-bottom:18px}.bbb-update-tabs{display:flex;gap:6px;flex-wrap:wrap}.bbb-update-tab{border:1px solid #264638;background:#0a120e;color:#9caaa3;padding:9px 11px;border-radius:9px;font-size:10px;font-weight:900}.bbb-update-tab.active{background:#0a8f4d;border-color:#0a8f4d;color:#fff}.bbb-update-search,.bbb-update-pos{width:100%;background:#050a08;border:1px solid #274539;color:#edf3ef;border-radius:9px;padding:10px 12px}.bbb-update-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.bbb-update-count{margin:15px 0;color:#71847a;font-size:10px}.bbb-update-more{display:block;margin:22px auto 0;border:1px solid #2e503f;background:#0d1a14;color:#d7e2dc;border-radius:9px;padding:10px 15px;font-size:11px;font-weight:900}.bbb-update-more.hide{display:none}.bbb-updates-empty{padding:50px 20px;text-align:center;color:#7f8d85;border:1px dashed #244236;border-radius:12px}
  @media(max-width:850px){.bbb-updates-home-grid,.bbb-update-grid{grid-template-columns:1fr}.bbb-update-controls{grid-template-columns:1fr}.bbb-updates-home-head,.bbb-updates-page-head{align-items:flex-start;flex-direction:column}.bbb-update-card.featured{min-height:0}}
  @media(max-width:560px){.bbb-updates-home{padding:45px 0}.bbb-updates-content{padding:38px 0 55px}.bbb-update-card,.bbb-update-card.featured{padding:16px}.bbb-update-card.featured .bbb-update-player h3{font-size:20px}.bbb-update-card.featured p{font-size:12px}}
...
function bbbRenderUpdatesHome(){
  ...
  const featured=bbbUpdates[0],rest=bbbUpdates.slice(1,5);
  box.innerHTML=`<div>${bbbUpdateCard(featured,true)}</div><div class="bbb-update-stack">${rest.map(x=>bbbUpdateCard(x)).join('')}</div>`;
}
