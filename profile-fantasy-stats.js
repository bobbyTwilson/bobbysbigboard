...
(function(){
  ...
  function seasonStatCards(pos,t){
    if(pos==='QB')return [
      ['CMP / ATT',`${int(t.completions)} / ${int(t.attempts)}`],
      ['PASS YDS',int(t.passing_yards)],
      ['PASS TD',int(t.passing_tds)],
      ['INT',int(t.interceptions)],
      ['RUSH YDS',int(t.rushing_yards)],
  ...
  function summaryHtml(player,rows,season){
    ...
    const production=seasonStatCards(pos,t);
    return `<section class="bbb-fantasy-season-strip" aria-label="${esc(season)} fantasy season summary">
      ...
      <div class="bbb-fantasy-production">
        <div class="bbb-fantasy-production-title">Season Production</div>
        <div class="bbb-fantasy-production-grid">${production.map(([label,value])=>`<div class="bbb-fantasy-production-stat"><span>${esc(label)}</span><strong>${esc(value)}</strong></div>`).join('')}</div>
      </div>
      <button type="button" class="bbb-fantasy-jump">View game log ↓</button>
    </section>`;
  }
  ...
  function bbbFantasyCleanCompare(){
  ...
  function bbbFantasyInstallCompareGuard(){
    const view=document.querySelector('#profileView');
    if(!view||view.dataset.bbbCompareGuard==='1')return;
    view.dataset.bbbCompareGuard='1';
    const observer=new MutationObserver(mutations=>{
      if(mutations.some(m=>m.addedNodes?.length))queueMicrotask(bbbFantasyCleanCompare);
    });
    observer.observe(view,{childList:true,subtree:true});
    bbbFantasyCleanCompare();
  }

  function bbbFantasyScrollToGameLog(){
  ...
  function render(slug,player,rows){
    ...
    summaryCard?.querySelector('.bbb-fantasy-jump')?.addEventListener('click',bbbFantasyScrollToGameLog);

    bbbFantasyCleanCompare();
    requestAnimationFrame(bbbFantasyCleanCompare);
    setTimeout(bbbFantasyCleanCompare,120);
    setTimeout(bbbFantasyCleanCompare,500);
    setTimeout(bbbFantasyCleanCompare,1200);
  }
  ...
  function injectStyles(){
    ...
    const s=document.createElement('style');s.id='bbb-fantasy-profile-styles';s.textContent=`
      ...
      @media(max-width:430px){.bbb-fantasy-season-strip{padding:12px}.bbb-fantasy-season-main strong{font-size:15px}.bbb-fantasy-season-main .rank strong{font-size:19px}.bbb-fantasy-production-grid{grid-template-columns:repeat(2,1fr)}.bbb-fantasy-production-stat{padding:8px 9px}.bbb-fantasy-production-stat strong{font-size:14px}}
    `;document.head.appendChild(s);
  }

  injectStyles();
  bbbFantasyInstallCompareGuard();
