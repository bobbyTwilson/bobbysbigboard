// Clearer separation between Bobby's ranking movement and consensus movement.
(function(){
  ...
  bbbV2TrendCard=function(m){
    ...
    const gapText=gap==null
      ...
      : gap>0
        ? `BBB ranks him ${Math.abs(gap)} spot${Math.abs(gap)===1?'':'s'} higher than consensus.`
        : gap<0
          ? `Consensus ranks him ${Math.abs(gap)} spot${Math.abs(gap)===1?'':'s'} higher than BBB.`
    ...
    return `<section class="profile-card full bbb-v2-trend-card bbb-v2-clarity-card">
      ...
      <div class="bbb-v2-card-head">
        <div>
          <h2>Your rank vs. consensus.</h2>
          <p>BBB movement shows changes Bobby made to the board. Consensus movement shows changes in the FantasyCalc market ranking.</p>
        </div>
      ...
      <div class="bbb-v2-clarity-explainer">Red or green movement inside the consensus panel only reflects the market changing. It does not mean Bobby moved the player on the Big Board.</div>
    </section>`;
  };
})();
